...
public class BlockBaseComponentImpl implements BlockBaseComponent {
    ...
    public boolean isDroppable(BlockStateWithPos blockState, ItemStack usedItem, Entity entity) {
        if (entity == null || (entity instanceof EntityPlayer player && player.getGameType() == GameType.CREATIVE)) return false;
    ...
    public boolean onInteract(BlockStateWithPos current, ItemStack itemStack, PlayerInteractInfo interactInfo) {
        var event = new CBlockOnInteractEvent(current, itemStack, interactInfo, false);
